...
namespace UnityEngine.AddressableAssets
{
    ...
    public static partial class AddressablesManager
    {
        ...
        public static async UniTask<OperationResult<T>> LoadAssetAsync2<T>(AssetReferenceT<T> reference) where T : Object
        {
        ...
        public static AsyncOperationHandle<IList<T>> LoadAssetsAsync<T>(object key) where T : Object
        {
            if (key == null)
            {
                Debug.LogError("Invalid key for loading assets.");
                return default; 
            }
            
            var operation = Addressables.LoadAssetsAsync<T>(key, asset =>
            {
                if (asset != null)
                {
                    string assetKey = asset.name;
                    Debug.Log(assetKey);
